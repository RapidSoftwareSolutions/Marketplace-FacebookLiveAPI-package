...
$app->post('/api/FacebookLiveAPI/getLiveVideoComments', function ($request, $response, $args) {
    ...
    $query_str = $settings['api_url'] . '/' . $post_data['args']['liveVideoId'] . '/comments';
    
    $query['access_token'] = $post_data['args']['accessToken'];
    
    if(!empty($post_data['args']['filter'])) {
        $query['filter'] = $post_data['args']['filter'];
    }
    if(!empty($post_data['args']['liveFilter'])) {
        $query['live_filter'] = $post_data['args']['liveFilter'];
    }
